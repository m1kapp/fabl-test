  ...
  return (
    <>
        ...
        <p className="kit-body">
          ...
          정답이 있는 문제는 <b>역량 체크</b> 쪽입니다. 그것도 20문항짜리 짧은 확인이에요.
        </p>
        <p className="kit-body">
          대신 <b>빈칸이 겹치는지</b>는 셀 수 있어요. 넷이 모였는데 시작 모드가 같으면
          그 힘만 강해지고, 모두가 마지막에 쓰는 힘이 팀 전체의 빈칸으로 남습니다.
        </p>
...
const EVIDENCE = [
  {
    ...
    body: '본인의 답 안에서 비중을 나눠 매기는 방식(ipsative)은 합이 구조적으로 고정돼, 두 사람의 점수를 나란히 놓고 누가 높은지 말할 수 없다. 이 테스트의 유형 점수가 정확히 그 방식이다. 이 논문은 그래서 순수 ipsative 도구를 쓰지 말 것을 권한다. 우리는 그 권고를 어기고 있고, 그래서 유형 결과를 판정이 아니라 이야깃거리로만 쓴다.',
    who: 'Hicks, L. E. (1970)',
  ...
  },
  {
    ...
    body: '집단의 성과를 예측한 것은 구성원 개개인의 능력 평균이 아니라, 발언 기회가 고르게 돌아가는지와 상대의 상태를 읽는 능력이었다. 이 연구는 성향 유형의 조합은 아예 다루지 않았다. 즉 유형을 골고루 섞으면 좋다는 근거도, 나쁘다는 근거도 여기서는 안 나온다.',
...
const LIMITS = [
  ...
  '공개 웹이라 재응시가 자유롭다. 문항은 매번 다시 뽑지만 풀이 한정돼 있어, 두 번째부터는 기억이 섞인다.',
  '유형 코드 세 글자는 첫 글자만큼 단단하지 않다. 채점 코드를 그대로 돌린 모의 실험에서, 같은 성향을 가정했을 때 12문항으로 세 글자가 그대로 다시 나오는 비율은 43% 안팎이다. 첫 글자 하나는 82% 선이다. 문항마다 제일 나중에 할 것까지 받는 이유가 이것인데, 그래도 세 글자보다 첫 글자를 믿는 편이 낫다.',
  '역량 체크는 회차마다 다른 20문항을 낸다. 응시자끼리 같은 시험을 본 것이 아니라서, 점수를 나란히 놓고 비교할 수 없다. 규준 집단도 없다.',
