...
function renderSharedType(code) {
  ...
  app.innerHTML = `<main class="intro landing shared-type">
    ...
    <section class="hero landing-hero"><div>
      ...
      <p class="lead"><b>${order[0].ko}</b>에서 시작해 <b>${order[1].ko}</b>을 거쳐 <b>${order[2].ko}</b> 모드로 완성하는 순서입니다.</p>
...
function renderIntro() {
  ...
  document.querySelector('#exampleResult').onclick = async () => {
    const payload = await fetch('/examples/minho-result.json').then(response => response.json());
    state = { ...createState(), ...payload.state, resultId: `example-${payload.state.resultId}`, isExample: true, screen: 'result' };
    render();
  };
